package tech.vvp.vvp.item.armor;

import tech.vvp.vvp.init.ModArmorMaterial;
import tech.vvp.vvp.client.renderer.armor.multicamhelmetRenderer;
import net.minecraft.world.item.Item;
import net.neoforged.api.distmarker.Dist;
import net.neoforged.api.distmarker.OnlyIn;
import software.bernie.geckolib.renderer.GeoArmorRenderer;

import java.util.function.Supplier;

public class multicamhelmet extends ModArmorMaterials {

    public multicamhelmet() {
        super(ModArmorMaterial.MULTICAM,
                Type.HELMET,
                new Properties().durability(Type.HELMET.getDurability(50)),
                0.2f
        );
    }

    @OnlyIn(Dist.CLIENT)
    @Override
    public Supplier<GeoArmorRenderer<? extends Item>> getRenderer() {
        return multicamhelmetRenderer::new;
    }
}